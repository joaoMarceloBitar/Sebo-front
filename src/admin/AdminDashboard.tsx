...
export default function AdminDashboard() {
  const [livrosAnuncio, setLivorsAnuncio] = useState<graficoLivroAnuncioType[]>([])
  ...
  useEffect(() => {
    ...
    async function getDadosGraficoMarca() {
      const response = await fetch(`${apiUrl}/dashboard/livrosComAnuncios`)
      const dados = await response.json()
      setLivorsAnuncio(dados)
    }
  ...
  const listaLivrosAnuncio = livrosAnuncio.map(item => (
